...
- 2026-07-28: TCP-relay blocker (`modules/nixos/efootball-block.nix` + waybar toggle) MECHANISM-VERIFIED live on legionix (gen in /run/current-system, unit present + inactive by default). Deterministic drop test via local listeners: port 34000 (inside 30000-35000) and 5736 both DROP (3s timeout); control port 36000 (outside range) connects instantly → collateral correctly scoped. Passwordless start AND stop via `systemctl` succeed (unit-scoped polkit rule for wheel works both ways); `nft -f` ExecStart exits 0/SUCCESS, table `inet efb` loads; ExecStop removes it (34000 reconnects after stop). Waybar wiring consistent (`signal = 9` matches `pkill -RTMIN+9`; self-hides where unit absent). NOT verifiable from shell: whether dropping these ports actually forces Konami matchmaking onto UDP/P2P — that's the in-game test (toggle on before matchmaking, play ranked, see if laggy TCP-relay matches vanish); if not, refresh port intel from upstream SuNingXJBT/eFootball_Block_TCP_Matches. Left service inactive (safe default).
