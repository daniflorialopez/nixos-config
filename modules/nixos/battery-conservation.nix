...
in
{
  ...
  # exposes a single conservation-mode toggle that caps charging at ~80% on
  # this Legion's firmware (measured 2026-07-29; the threshold is fixed in
  # firmware — there is no charge_control_*_threshold node to tune it),
  ...
  # conservation by default, since capping at ~80% is the right daily state
  ...
  systemd.services.battery-conservation = {
    description = "Lenovo battery conservation: open the toggle to wheel, cap charge ~80%";
